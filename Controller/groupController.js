...
async function extaract(id) {
  try {
    const group = await Group.findById(id);
    const groupMembers = group.groupMembers.flatMap((el) => el.number);
    const token = await Token.find();
    const existingUsers = token.filter((el) => groupMembers.includes(el.phone));

    const nonExistingUsers = groupMembers.filter(
      (el) => !existingUsers.includes(el)
    );
...
exports.createGroup = async (req, res, next) => {
  try {
    ...
    await user.save();
    const { existingUsers, nonExistingUsers } = await extaract(group._id);
